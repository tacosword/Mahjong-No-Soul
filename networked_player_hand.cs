...
public class NetworkedPlayerHand : NetworkBehaviour
{
    ...
    /// <summary>
    /// Initialize interrupt UI - finds it even if GameObject is inactive.
    /// Uses Resources.FindObjectsOfTypeAll to find inactive GameObjects.
    /// </summary>
    private void InitializeInterruptUI()
    {
        Debug.Log("╔════════════════════════════════════════════════════════════╗");
        Debug.Log("║  [PlayerHand] InitializeInterruptUI CALLED                 ║");
        Debug.Log("╚════════════════════════════════════════════════════════════╝");
        
        if (interruptUI != null)
        {
            Debug.Log("[PlayerHand] InterruptUI already assigned, skipping");
            return;
        }
        
        // CRITICAL FIX: Resources.FindObjectsOfTypeAll finds INACTIVE objects
        // This is why FindFirstObjectByType was failing - InterruptPanel was inactive!
        InterruptUIManager[] allManagers = Resources.FindObjectsOfTypeAll<InterruptUIManager>();
        
        Debug.Log($"[PlayerHand] Found {allManagers.Length} InterruptUIManager(s) total (including inactive)");
        
        foreach (InterruptUIManager mgr in allManagers)
        {
            // Filter out prefabs - we only want scene objects
            // Prefabs have null or empty scene names
            if (mgr.gameObject.scene.name == null || mgr.gameObject.scene.name == "")
            {
                Debug.Log($"[PlayerHand] Skipping InterruptUIManager on prefab: {mgr.gameObject.name}");
                continue;
            }
            
            Debug.Log($"[PlayerHand] ✓ Found InterruptUIManager in scene!");
            Debug.Log($"[PlayerHand]     GameObject: {mgr.gameObject.name}");
            Debug.Log($"[PlayerHand]     Active in hierarchy: {mgr.gameObject.activeInHierarchy}");
            Debug.Log($"[PlayerHand]     Active self: {mgr.gameObject.activeSelf}");
            Debug.Log($"[PlayerHand]     Scene: {mgr.gameObject.scene.name}");
            
            interruptUI = mgr;
            
            // Initialize it with this player hand
            interruptUI.Initialize(this);
            
            Debug.Log("╔════════════════════════════════════════════════════════════╗");
            Debug.Log("║  [PlayerHand] ✓✓✓ InterruptUI SUCCESSFULLY INITIALIZED     ║");
            Debug.Log("╚════════════════════════════════════════════════════════════╝");
            return;
        }
        
        Debug.LogError("╔════════════════════════════════════════════════════════════╗");
        Debug.LogError("║  [PlayerHand] ✗✗✗ CRITICAL ERROR ✗✗✗                     ║");
        Debug.LogError("║  InterruptUIManager NOT FOUND IN SCENE (even inactive)    ║");
        Debug.LogError("╚════════════════════════════════════════════════════════════╝");
    }
    ...
    void Start()
    {
        ...
        // ┌──────────────────────────────────────────────────────────────┐
        // │ ADD THIS LINE - Initialize InterruptUI                       │
        // └──────────────────────────────────────────────────────────────┘
        InitializeInterruptUI();
        // ┌──────────────────────────────────────────────────────────────┐
    ...
    public void TargetCheckInterruptOptions(NetworkConnection target, int canChiFlag, int canPonFlag, int canKongFlag, int discardedTileSortValue)
    {
        ...
        // CRITICAL FIX: Find InterruptUI if null - use Resources to find inactive objects
        if (interruptUI == null)
        {
            Debug.LogWarning($"[PlayerHand] InterruptUI was null! Searching for it now...");
            
            InterruptUIManager[] allManagers = Resources.FindObjectsOfTypeAll<InterruptUIManager>();
            Debug.Log($"[PlayerHand] Found {allManagers.Length} InterruptUIManager(s) in runtime search");
            
            foreach (InterruptUIManager mgr in allManagers)
            {
                // Filter out prefabs - only want scene objects
                if (mgr.gameObject.scene.name != null && mgr.gameObject.scene.name != "")
                {
                    interruptUI = mgr;
                    interruptUI.Initialize(this);
                    Debug.Log($"[PlayerHand] ✓ Found and initialized InterruptUI: {mgr.gameObject.name}");
                    break;
                }
            }
            ...
            if (interruptUI == null)
            {
                Debug.LogError($"[PlayerHand] ✗ CRITICAL: Still could not find InterruptUIManager!");
            }
        }
    ...
    public void OnInterruptDecision(InterruptActionType decision)
    {
        ...
        if (myPlayer != null)
        {
            Debug.Log($"[PlayerHand] My PlayerIndex: {myPlayer.PlayerIndex}");
            Debug.Log($"[PlayerHand] My Username: {myPlayer.Username}");
            Debug.Log($"[PlayerHand] My IsOwned: {myPlayer.isOwned}");
            Debug.Log($"[PlayerHand] My IsServer: {isServer}");
            Debug.Log($"[PlayerHand] My IsClient: {isClient}");
        }
    ...
    {
        Debug.Log("╔════════════════════════════════════════════════════════════╗");
        Debug.Log($"║ [PlayerHand] CmdRespondToInterrupt ON SERVER              ║");
        Debug.Log("╚════════════════════════════════════════════════════════════╝");
        
        NetworkPlayer myPlayer = GetComponent<NetworkPlayer>();
        if (myPlayer == null)
        {
            Debug.LogError("[PlayerHand] ✗ Cannot get NetworkPlayer component!");
            return;
        }
        
        int myPlayerIndex = myPlayer.PlayerIndex;
        string myUsername = myPlayer.Username;
        bool myIsOwned = myPlayer.isOwned;
        
        Debug.Log($"[PlayerHand] Player Info:");
        Debug.Log($"[PlayerHand]   Username: {myUsername}");
        Debug.Log($"[PlayerHand]   PlayerIndex: {myPlayerIndex}");
        Debug.Log($"[PlayerHand]   IsOwned: {myIsOwned}");
        Debug.Log($"[PlayerHand]   Action: {action}");
        
        if (action == InterruptActionType.Chi && selectedChiOption != null)
        {
            Debug.Log($"[PlayerHand] Storing Chi option: {selectedChiOption.tile1SortValue}, {selectedChiOption.tile2SortValue}, {selectedChiOption.discardedTile}");
            CmdStoreChiOption(selectedChiOption.discardedTile, selectedChiOption.tile1SortValue, selectedChiOption.tile2SortValue);
        }
        
        if (NetworkedGameManager.Instance != null)
        {
            Debug.Log($"[PlayerHand] ✓ Calling GameManager.PlayerRespondedToInterrupt({myPlayerIndex}, {action})");
            NetworkedGameManager.Instance.PlayerRespondedToInterrupt(myPlayerIndex, action);
        }
        else
        {
            Debug.LogError("[PlayerHand] ✗ NetworkedGameManager.Instance is NULL!");
        }
    }
